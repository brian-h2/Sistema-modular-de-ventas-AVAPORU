...
export default function Stock() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.stockMinimo > form.stockDisponible) {
      Swal.fire({
        icon: 'warning',
        title: 'Stock mínimo inválido',
        text: 'El stock mínimo no puede ser mayor al stock disponible.'
      });
      return;
    }

    const trimmedForm = { ...form, sku, nombre, categoria };
    setLoading(true);

    try {
      if (editingProduct) {
        await updateProduct(editingProduct._id, trimmedForm);
        ...
        await createProduct({
          ...
          precio: trimmedForm.precio,
          stockDisponible: trimmedForm.stockDisponible,
          stockMinimo: trimmedForm.stockMinimo,
          categoria: trimmedForm.categoria,
          imageUrl: trimmedForm.imageUrl,
        });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 dark:from-slate-900 to-slate-100 dark:to-slate-800 p-4 sm:p-8 font-sans">
      {/* Header */}
      <motion.div 
      ...
      >
        <div>
          <div className="flex items-center gap-3 mb-1">
            <div className="p-2 bg-teal-100 dark:bg-teal-900/40 text-teal-600 dark:text-teal-400 rounded-xl shadow-sm">
              <Package className="w-6 h-6" />
            </div>
            <h1 className="text-4xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight">Gestión de Stock</h1>
          </div>
      ...
      >
        <motion.div variants={itemVariants} className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-5 flex items-center gap-3 transition-all hover:-translate-y-1 hover:shadow-md">
          <div className="p-2 bg-teal-100 dark:bg-teal-900/40 rounded-lg">
            <CubeIcon className="h-7 w-7 text-teal-600 dark:text-teal-400" />
          </div>
          <div>
        ...
        <motion.div variants={itemVariants} className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-5 flex items-center gap-3 transition-all hover:-translate-y-1 hover:shadow-md">
          ...
          <div>
            <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Stock Normal</p>
        ...
        <motion.div variants={itemVariants} className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-5 flex items-center gap-3 transition-all hover:-translate-y-1 hover:shadow-md">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-yellow-600 dark:text-yellow-400">{stats.bajo}</p>
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-5 flex items-center gap-3 transition-all hover:-translate-y-1 hover:shadow-md">
          ...
          <div>
            <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Crítico/Sin Stock</p>
            <p className="text-2xl font-bold text-red-600 dark:text-red-400">{stats.sinStock}</p>
          </div>
        </motion.div>
      </motion.div>
      ...
      {/* Filters */}
      <motion.div 
        className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-5 mb-6"
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-semibold text-gray-900 dark:text-slate-100">Filtros y Búsqueda</h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="relative">
            <MagnifyingGlassIcon className="h-5 w-5 text-gray-400 dark:text-slate-500 absolute left-3 top-1/2 transform -translate-y-1/2" />
            <input
              type="text"
              placeholder="Buscar por código, nombre o marca..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent bg-slate-50/50 dark:bg-slate-800/50 transition-colors"
            />
          </div>
          ...
          <select
            ...
            className="px-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent bg-slate-50/50 dark:bg-slate-800/50 transition-colors"
          >
            {categories.map((cat) => (
      ...
      {/* Products Table */}
      <motion.div 
        className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden"
        initial={{ opacity: 0, y: 15 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.4, duration: 0.4 }}
      >
        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
          <h2 className="font-semibold text-gray-900 dark:text-slate-100">Inventario Actual</h2>
        </div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <div className="flex items-center gap-2">
                          <motion.button
                            ...
                            className="text-gray-600 dark:text-slate-400 hover:text-gray-900 hover:dark:text-slate-100 p-1.5 rounded-lg hover:bg-slate-100 hover:dark:bg-slate-800 transition-colors"
      ...
      <AnimatePresence>
        {showModal && (
          ...
          >
            <motion.div 
              className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-slate-100 dark:border-slate-700"
            ...
            >
              <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
                ...
                <motion.button
                  ...
                  className="text-gray-400 dark:text-slate-500 hover:text-gray-600 hover:dark:text-slate-400 p-1 rounded-lg hover:bg-slate-100 hover:dark:bg-slate-800 transition-colors"
                  whileHover={{ scale: 1.1 }}
                  whileTap={{ scale: 0.9 }}
                >
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div className="flex gap-3 pt-4">
                  <motion.button
                    type="button"
                    onClick={handleCloseModal}
                    className="flex-1 px-4 py-2.5 border border-slate-300 dark:border-slate-600 text-gray-700 dark:text-slate-300 rounded-xl hover:bg-slate-50 hover:dark:bg-slate-950 transition-colors font-medium"
