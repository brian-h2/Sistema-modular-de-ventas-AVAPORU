// ====================================================================
// SCRIPT DE COPIAR Y PEGAR EN MONGODB COMPASS (MONGOSH)
// ====================================================================
// Abre MongoDB Compass -> haz clic en "_MONGOSH" (consola inferior) -> pega todo este código y presiona Enter.
...
(() => {
  ...
  const usersToInsert = [
    { _id: new ObjectId(), email: "carlos.vendedor@avaporu.com", nombre: "Carlos Pérez", role: "Vendedor", status: "active", passwordHash: "$2a$10$e8w5hH4...hashed", createdAt: new Date(), updatedAt: new Date() },
    { _id: new ObjectId(), email: "ana.vendedora@avaporu.com", nombre: "Ana Martínez", role: "Vendedor", status: "active", passwordHash: "$2a$10$e8w5hH4...hashed", createdAt: new Date(), updatedAt: new Date() },
    { _id: new ObjectId(), email: "roberto.encargado@avaporu.com", nombre: "Roberto Fernández", role: "Encargado", status: "active", passwordHash: "$2a$10$e8w5hH4...hashed", createdAt: new Date(), updatedAt: new Date() },
    { _id: new ObjectId(), email: "lucia.gerente@avaporu.com", nombre: "Lucía Gómez", role: "Gerente", status: "active", passwordHash: "$2a$10$e8w5hH4...hashed", createdAt: new Date(), updatedAt: new Date() }
  ];
  ...
  if (existingUsers.length === 0) {
    ...
    print(`ℹ️ Se utilizarán los ${existingUsers.length} usuarios existentes`);
  }

  // 2. PRODUCTOS (55 CALZADO + 35 ACCESORIOS)
  const PRODUCTS_RAW = [
  ...
  print(`✅ Insertados ${productsToInsert.length} productos en la colección 'products'`);

  // 3. GENERAR VENTAS CON VENDEDOR Y FECHAS
  const salesToInsert = [];
  for (let i = 0; i < 250; i++) {
  ...
  print(`✅ Insertadas ${salesToInsert.length} ventas en la colección 'sales'`);
  ...
  print(`✅ Insertados ${expensesToInsert.length} gastos en la colección 'expenses'`);
